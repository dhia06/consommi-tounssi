...
@ELBeanName(value = "promoController")
@Join(path = "/admin/managePromotion", to = "/pages/adminSide/promos/addPromos.jsf")
public class PromosController implements Serializable {
	/**
	 * 
	 */
	...
	private List<PromosMagazin> listActivePromo = new ArrayList<PromosMagazin>();
	...
	public List<PromosMagazin> getListActivePromo() {
		listActivePromo.clear();
		List<PromosMagazin> listPromo = promosService.getAllPromos();
		for(PromosMagazin p :listPromo){
			if(p.getImage()!=null && p.getStatus().equals("active") )
				listActivePromo.add(p);
			
		}
		return listActivePromo;
	}

	public void setListActivePromo(List<PromosMagazin> listActivePromo) {
		this.listActivePromo = listActivePromo;
	}
	...
	public List<PromosMagazin> getListPromos() {
		...
		System.out.println("getListPromos"+listPromos);
		return listPromos;
	}
	...
	public String addThePromos(){
		...
		System.err.println("addThePromos dd1"+date1);
		System.err.println("addThePromos dd2"+date2);
